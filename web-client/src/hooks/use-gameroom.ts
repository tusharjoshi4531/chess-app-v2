...
export const useGameRoom = () => {
  ...
  useEffect(() => {
    ...
    const receiveRoomData = (error: unknown | undefined, data: IRoomData) => {
      if (error) {
  ...
  const sendMoveHandler = (fen: string, turn: "w" | "b") => {
    ...
    socket.emit("room/send-move", { roomid, fen, turn }, (error: unknown) => {
      if (error) {
  ...
  const sendResign = (username: string) => {
    if (!socket || !roomid || roomState.finished) return;
    socket.emit("room/send-resign", { roomid, username }, (error: unknown) => {
      if (error) {
  ...
  const sendDraw = (username: string) => {
    if (!socket || !roomid || roomState.finished) return;
    socket.emit("room/send-draw", { roomid, username }, (error: unknown) => {
      if (error) {
  ...
  const sendTimeout = (username: string) => {
    if (!socket || !roomid || roomState.finished) return;
    socket.emit("room/send-timeout", { roomid, username }, (error: unknown) => {
      if (error) {
